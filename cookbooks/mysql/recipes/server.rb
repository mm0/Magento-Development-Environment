...
#TODO: create magento user 
execute "create-magento-db" 
	command "\"#{node['mysql']['mysql_bin']}\" -u root -p '#{node['mysql']['server_root_password']}' -e \"CREATE DATABASE #{node['mysql']['server_magento_database_name']}; GRANT USAGE ON *.* TO #{node['mysql']['server_magento_database_username']} localhost IDENTIFIED BY '#{node['mysql']['server_magento_database_password']}'; GRANT ALL PRIVILEGES ON #{node['mysql']['server_magento_database_name']}.* TO #{node['mysql']['server_magento_database_username']}@localhost; FLUSH PRIVILEGES;\" "
	action :run
	only_if "\"#{node['mysql']['mysql_bin']}\" -u root -e 'show databases;'"
end
